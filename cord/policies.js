...
module.exports = {
  ...
  regex: {
    injection:
      /(<<|{{|}}\s*--|;\s*--|rm\s+-rf|\bunion\b|\bdrop\b|\binsert\b|\bexec\b|\bshutdown\b|curl\s+https?:|\beval\b|\bimport\s+os\b|\bsubprocess\b|__import__|os\.system|os\.popen)/i,
    exfil:
      /(upload|exfil|curl\s+https?:|wget\s+https?:|scp\s|nc\s|requests\.post|urllib|httpx\.post|send.*to.*server|transmit|beacon)/i,
    secrets:
      /(api[_\- ]?keys?|tokens?|passwords?|secrets?\b|aws_secret|authorization|credentials?|private[_\- ]?keys?|ssh[_\- ]?keys?|\.env\b|keychain|passphrase|system\s+prompts?)/i,
    ...
    piiFieldNames:
      /\b(social_security|ssn|credit_card|card_number|cvv|date_of_birth|dob|passport|drivers_license|medical_record|bank_account|routing_number|tax_id)\b/i,
  },
