...
function tempLog() {
  return path.join(
    os.tmpdir(),
    `vigil-test-${Date.now()}-${Math.random().toString(36).slice(2)}.jsonl`,
  );
}
...
describe("CanaryRegistry.scan() — detection", () => {
  ...
  test("clean output does not trigger", () => {
    ...
    const result = registry.scan(
      "Here is a helpful response about sorting algorithms.",
    );
